...
func get_nav_path(start: Vector2, end: Vector2):
  # Convert positions to cell coordinates
  var start_tile = world_to_map(start)
  var end_tile = world_to_map(end)

  # Determines IDs
  var start_id = astar.get_closest_point(Vector3(start_tile.x, start_tile.y, 0))
  var end_id = astar.get_closest_point(Vector3(end_tile.x, end_tile.y, 0))
